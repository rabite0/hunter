...
impl Cmd {
    ...
    fn substitute_cwd_files(&mut self, cmd: Vec<OsString>) -> Vec<OsString> {
        ...
        cmd.iter()
            .map(|part| part.splice_quoted(&cwd_pat, cwd_files.clone()))
            .flatten()
            .collect()
    }
    ...
    fn substitute_tab_files(&mut self, cmd: Vec<OsString>) -> Vec<OsString> {
        ...
        tab_files
            ...
            .fold(cmd, |cmd, (i, tab_files)| {
                ...
                let tab_file_paths = tab_files
                    .iter()
                    .map(|file| file.strip_prefix(&self.cwd).into_os_string())
                    .collect::<Vec<OsString>>();

                cmd.iter()
                    .map(|part| part.splice_quoted(&tab_files_pat, tab_file_paths.clone()))
                    .flatten()
                    .collect()
            })
    }
    ...
    fn substitute_tab_paths(&mut self, cmd: Vec<OsString>) -> Vec<OsString> {
        ...
        tab_paths
            ...
            .fold(cmd, |cmd, (i, tab_path)| {
                ...
                cmd.iter()
                    .map(|part| part.splice_quoted(&tab_path_pat, vec![tab_path.clone()]))
                    .flatten()
                    .collect()
            })
    }
...
impl Widget for ProcView {
    ...
    fn on_key(&mut self, key: Key) -> HResult<()> {
        match key {
            ...
            Key::Char('f') => {
                self.toggle_follow().log();
            }
            Key::Ctrl('n') => {
                self.scroll_down().log();
            }
            Key::Ctrl('p') => {
                self.scroll_up().log();
            }
...
impl ConcatOsString for Vec<OsString> {
    fn concat(&self) -> OsString {
        ...
        self.iter()
            .enumerate()
            .fold(OsString::new(), |string, (i, part)| {
